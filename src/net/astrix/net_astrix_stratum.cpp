...
int handleAstrixStratumPacket(boost::json::object packet, AstrixStratum::jobCache *cache, bool isDev)
{
  ...
  if (M.compare(AstrixStratum::s_notify) == 0)
  {
    ...
    AstrixStratum::lastReceivedJobTime = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now().time_since_epoch()).count();


    (*J).as_object()["template"] = std::string(newTemplate, AstrixHash::INPUT_SIZE*2);
  ...
  else if (M.compare(AstrixStratum::s_print) == 0)
  {

    int lLevel = packet.at("params").as_array()[0].to_number<int64_t>();
    if (lLevel != AstrixStratum::STRATUM_DEBUG)
    {
      int res = 0;
      printf("\n");
      if (isDev)
      ...
      switch (lLevel)
      {
      case AstrixStratum::STRATUM_INFO:
        if (!isDev)
          setcolor(BRIGHT_WHITE);
        printf("Stratum INFO: ");
        break;
      case AstrixStratum::STRATUM_WARN:
        if (!isDev)
          setcolor(BRIGHT_YELLOW);
        printf("Stratum WARNING: ");
        break;
      case AstrixStratum::STRATUM_ERROR:
        if (!isDev)
          setcolor(RED);
        printf("Stratum ERROR: ");
        res = -1;
        break;
      case AstrixStratum::STRATUM_DEBUG:
        break;
      }
...
int handleAstrixStratumResponse(boost::json::object packet, bool isDev)
{
  // if (!isDev) {
  ...
  switch (id)
  {
    case AstrixStratum::subscribeID:
    {
      std::cout << boost::json::serialize(packet).c_str() << std::endl;
    ...
    bool isDev)
{
  ...
  AstrixStratum::jobCache jobCache;

  // Subscribe to Stratum
  packet = AstrixStratum::stratumCall;
  packet["id"] = AstrixStratum::subscribe.id;
  packet["method"] = AstrixStratum::subscribe.method;
  ...
  try {
    ...
    for (std::string packet : packets) {
      ...
      {
        handleAstrixStratumPacket(subRPC, &jobCache, isDev);
      } 
    }
  ...
  packet = AstrixStratum::stratumCall;
  packet.at("id") = AstrixStratum::authorize.id;
  packet.at("method") = AstrixStratum::authorize.method;
  ...
  try {
    ...
    for (std::string packet : packets) {
      ...
      {
        handleAstrixStratumPacket(authRPC, &jobCache, isDev);
      }
    }
  } catch (const std::exception &e) {
    setcolor(RED);
  ...
  // AstrixStratum::stratumCall;
  // packet.at("id") = AstrixStratum::subscribe.id;
  // packet.at("method") = AstrixStratum::subscribe.method;
    ...
    while(!abort) {
      ...
      try {
        ...
        boost::asio::async_write(stream, boost::asio::buffer(msg), [&](const boost::system::error_code& error, std::size_t bytes_transferred) {
          ...
          if (!isDev) AstrixStratum::lastShareSubmissionTime = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
          // (*B) = false;
          // data_ready = false;
        });
        (*B) = false;
  ...
  while (true)
  {
    ...
    try
    {
      if (
          ...
          std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now().time_since_epoch()).count() - AstrixStratum::lastReceivedJobTime > AstrixStratum::jobTimeout)
      {
        setcolor(RED);
        printf("timeout\n");
        fflush(stdout);
      ...
      if (trans > 0)
      {
        ...
        for (std::string packet : packets) {
          try {
            ...
            {
              if (std::string(sRPC.at("method").as_string().c_str()).compare(AstrixStratum::s_ping) == 0)
              {
                boost::json::object pong({{"id", sRPC.at("id").get_uint64()},
                                          {"method", AstrixStratum::pong.method}});
                ...
                handleAstrixStratumPacket(sRPC, &jobCache, isDev);
            }
            else
            {
              handleAstrixStratumResponse(sRPC, isDev);
            } 
          } catch(const std::exception &e){
